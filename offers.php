...
##########################
if (isset($_GET['offers_type'])) {
    mysqli_query($con, "UPDATE " . $prefix . "_config SET offers_type=" . $_GET['offers_type'] . " where id=" . $get_db_id . "");
//    header("refresh:0;url=offers.php");
}
####################################
mysqli_query($con, "UPDATE ".$prefix."_offers_temporary SET type='1' where user_id='$user_id' and type='2'");
mysqli_query($con, "UPDATE ".$prefix."_offers_temporary SET Quantity=Quantity*-1 where user_id='$user_id' and Quantity<=0");
mysqli_query($con, "UPDATE ".$prefix."_offers_temporary SET Total=Total*-1 where user_id='$user_id' and Total<=0");
if (isset($_POST['date'])) {
    $_POST['date'] = str_replace("/", "-", $_POST['date']);
    $DueDate = Trim(date('Y-m-d', strtotime($_POST['date'])));
    ##################
    if (mysqli_query($con, "UPDATE " . $prefix . "_offers_temporary SET  date='$DueDate' where user_id=$user_id")) {
    }
}
//if($_POST['inv_tax']=='1'){
                ...
                <div style="width:60%;border:1px solid #CCC; border-radius:5px; float:right; ">
                    <div>
                       ...
                       <a class="addProd" par="q" attr="d"><img src="images/c.png" style="width:30px; height:30px; float:left; border:0px;"  title="<?php /*echo"$Cancel_lang"; */?>"/></a>
                        ...
                        <?php
//                        if ($Retail_allow == "1") {
//                            if ($get_db_offers_type == "1") {
//                                echo' <a href="offers.php?offers_type=2"><img src="images/Shopping_retail-04-128.png" style="width:30px; height:30px; float:left; border:0px;" title="'.$Retail_lang.'"   /></a>';
//                            } else if ($get_db_offers_type == "2") {
//                                echo'<a href="offers.php?offers_type=1"><img src="images/g.png" style="width:30px; height:30px; float:left; border:0px;" title="'.$Wholesaling_lang.'" /></a>';
//                            } else {
//                                echo'<a href="offers.php?offers_type=1"><img src="images/g.png" style="width:30px; height:30px; float:left; border:0px;" title="'.$Wholesaling_lang.'" /></a>';
//                            }
//                        }
                        ...
                        if ($_GET['print_inv'] !== null) {
                            ##############
                            $result_Get_supplier = mysqli_query($con, "SELECT  supplier  FROM ".$prefix."_offers_inv where  inv_id=".$_GET['print_inv']."");
                            ...
                            <!--                                        <!--<a href="#" onclick="javascript:void window.open('pdf/export/offers_inv.php?id=-->
                            <!--                                        --><?php /*//echo"" . $_GET['print_inv'] . ""; */?>
                            <!--                                                &type=offers', '13909375026416', 'width=700,height=600,toolbar=0,menubar=0,location=0,status=0,scrollbars=1,resizable=0,left=0,top=0,menubar=0');-->
                        ...
                        <?php echo"$Bill_of_offers_lang"; ?></div>
...
//                        var_dump($_POST['submit']);
                        if (($_POST['submit']) == $Save_lang and $_POST['submit'] != null) {
                            ...
                            $result_upt = mysqli_query($con, "SELECT * FROM " . $prefix . "_offers_temporary where user_id='$user_id' order by id DESC");
...
//
//                                    $sqlpayments_suppliers = "INSERT INTO " . $prefix . "_offers_inv (name, Total, date ,user_id)
//VALUES ('" . $_POST['name']  . "','" . $_POST['total'] . "','" .  $DueDate  . "','" . $user_id . "')";
...
//                                           $sqlpayments = "INSERT INTO " . $prefix . "_offers_inv (name, Total, date, user_id)
//VALUES ('" .  $_POST['name']  . "','"  . $_POST['total']  . "','"  . $DueDate ."','" .  $user_id . "')";
...
//                                    }
                                ...
                                ############################
                                mysqli_query($con, "UPDATE " . $prefix . "_config SET  LastoffersInvoiceNo= LastoffersInvoiceNo + 1 where id=" . $get_db_id . "");

                                mysqli_query($con, "select LastoffersInvoiceNo FRom  " . $prefix . "_config  where id=" . $get_db_id . "");
                                $configSql =                                 "SELECT LastoffersInvoiceNo FROM " . $prefix . "_config where id='1'"
                                ;
                                $result_LastInvoiceNo = mysqli_query($con,
                                    $configSql                            );
                                while ($row_LastInvoiceNo = mysqli_fetch_array($result_LastInvoiceNo)) {
                                    $get_LastInvoiceNo = $row_LastInvoiceNo['LastoffersInvoiceNo'];
                                }
######################
                                ######################
                                $result_up = mysqli_query($con, "SELECT * FROM " . $prefix . "_offers_temporary where user_id='$user_id' order by id DESC");
                                while ($row_up = mysqli_fetch_array($result_up)) {
                                    //echo $row_up['id'];
                                    $item = round($_POST[item . $row_up['id']]);
                                    $quantity = $_POST[quantity . $row_up['id']];

                                     $update_temp_sqlss="UPDATE " . $prefix . "_offers_temporary SET inv_id='" . $get_LastInvoiceNo  . "',Quantity='" . $quantity .  "',date='" . $DueDate ."' where user_id='$user_id' and id='" . $row_up['id'] . "'";
...
//                                if($db_tax>0){$add_tax_inv=($inv_Total-$_POST['alldiscount'])*$db_tax/100;}else{$add_tax_inv=0;}
                                   $insertSql  = "INSERT INTO " . $prefix . "_offers_inv(name,inv_id, date, Total, user_id)
VALUES ('" . $_POST['name']   .    "','". $get_LastInvoiceNo . "','" . $DueDate . "','" . $_POST['total'] ."','" . $user_id . "')";
                                if (mysqli_query($con, $insertSql)) {
                                    $Report_Create_Invoice = 1;
##############
                                      $offerSql = "INSERT INTO " . $prefix . "_offers(Quantity, date, inv_id, item, user_id)
SELECT Quantity  ,date,inv_id,item,user_id from " . $prefix . "_offers_temporary where user_id='$user_id' and inv_id !=''" ;
...
################
                                ################################
                                if (mysqli_query($con, "DELETE FROM " . $prefix . "_offers_temporary where user_id='$user_id'")) {
//                                    header("refresh:1;url=offers.php?print_inv=" . $get_LastInvoiceNo . "");
                                }else{
                                    $mysqli_errno++;
                                }
                                //echo $inv_Total;
                        ...
                        <?php if (isset($_GET['SupplierID']) and $_GET['SupplierID'] !== null) {
                            ...
                            if (mysqli_query($con, "UPDATE ".$prefix."_offers_temporary SET SupplierID=".$SupplierID." where user_id='$user_id'")) {
//                                header("refresh:1;url=offers.php");
                            } else {
...
####################
                            if ($_GET['barcode'] == null) {
                                ...
                                if (@mysqli_num_rows($result_new) >= 1) {
                                    ...
                                    $sql = "INSERT INTO " . $prefix . "_offers_temporary (inv_id , name,item, Quantity,  date, user_id)
							VALUES ('" . $_POST['name'] . "','". $item_id_new  . "','1','" . $DueDate . "','".$user_id."')";
                                    }
                        ...
                        <form method="post" id="inv_form">
                            ...
                            <div class="row" dir="rtl">
                                <div class="col-12" >
                                    ...
                                    <input type="text" placeholder=" <?php echo"$the_name_lang"; ?>" name="name" value="<?php echo"" . $_POST['name'] . ""; ?>"  />
                                    <br/>
                                    <br/>
                                    <br/>
                                </div>
...
<div id="ajaxRes">
                            ...
                            <table  border="1" style="font-size:16px; width:100%;  direction:rtl; border:1px; border-collapse:collapse; margin-top:10px; text-align:center;">
                                ...
                                $sql = "SELECT * FROM " . $prefix . "_offers_temporary where user_id='$user_id' order by id DESC";
                                $result = @mysqli_query($con, $sql);
                                $tbl_name = "" . $prefix . "offers_temporary";
                                //your table name
                                 ...
                                 */
                                $query = "SELECT COUNT(*) as num  FROM  " . $prefix . "_offers_temporary where user_id='$user_id' order by id DESC";
...
//
//                                                    if ($row['mobile1']!= ''){
//                                                        $_POST['mobile1'] = $row['mobile1'] ;
//                                                    } if ($row['mobile2']!= ''){
//                                                        $_POST['mobile2'] = $row['mobile2'] ;
//                                                    } if ($row['region_id']!= ''){
...
###############################################################################
							...
							<td><input type="hidden" name="item' . $row['id'] . '" value="' . $item_id . '" />' . $item_name . '</td>';
                                    ...
                                    echo'<td valign="middle"><a par="del" attr="'.$row['id'].'" class="addProd"><img src="images/erase.png" style="border:0px;" /></a></td>
							</tr>';
                                    $i++;
                                }
                                ?>
...
<td>
    <label><?php echo $the_total_lang;?></label></td>
                            <td>
    <input type="text" placeholder=" <?php echo"$the_total_lang"; ?>" name="total" value="<?php echo"" . $_POST['total'] . ""; ?>"  />
...
<a class='addProd' par='cat_show' attr='".$row_cat['id']."' class='a_cat_underlines'><img src=\"uploads/" . $row_cat['image'] . "\" class=\"img-responsive\" width=\"115\" height=\"30\" /></a></div>
                    ...
                    <div id="items">
                        ...
                        while ($row = @mysqli_fetch_array($result)) {
							...
							<div class=\"jqx-rc-t draggable-demo-product-header jqx-widget-header-theme1 jqx-fill-state-normal-theme\" >
							<div class=\"draggable-demo-product-header-label\" style='background-color:#fff;'> <a par='q' attr='".$row['id']."' class='addProd'>" . $row['item'] . "</a></div></div>
    ...
    <script>
        ...
        $(document).on("change",'.gr,.gr2',function() {
            ...
            $.ajax({
                url: 'ajax/offer_add_product_temp.php',
                dataType: 'text',
                type: 'POST',
