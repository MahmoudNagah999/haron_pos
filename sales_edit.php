...
if (isset($_GET['cat_show'])) {
    ...
    header("refresh:0;url=sales_edit.php?id=".$_GET['id']."");
}
?>
<!DOCTYPE html>
<html lang="en">
    <head>
        ...
        <script>
            function showResults(str) {
                ...
                xmlhttp.open("GET", "clients_search.php?q=" + str+"&id=<?php echo $_GET['id'] ; ?>&status=edit"  , true);
                xmlhttp.send();
            }
        </script>
        <script>
                            ...
                            <div>
                                ...
                                <div style="width:100%; padding-top:0px; text-align:center; margin:0 auto;border:0px dashed #CCC; border-radius:5px; height:290px; overflow:auto;">
                                    ...
                                    if (($_POST['submit']) == $Save_lang and $_POST['submit'] != null) {
                                        ...
                                        $result_upt = mysqli_query($con, "SELECT * FROM " . $prefix . "_sales where inv_id='$inv_id' order by id DESC");
...
//                                        if ($_POST['SupplierID'] == "" and $paid != $total_inv_after_alldiscount_and_tax) {
							...
							'.$musts_choose_customer.'
							</div>';
                                        } else
                                        if ($_POST['safe_id'] == ""  and  $paid > 0){
							...
							'.$must_choose_store.'
                                            ...
                                            while ($row_up = mysqli_fetch_array($result_up)) {
                                                ...
                                                $inv_Total+=($quantity * $Price) - $DiscountValue;
                                                  ...
                                                  $sql = "UPDATE " . $prefix . "_sales SET staff='" . $staff . "',Quantity='" . $quantity . "',
...
####
                                    ...
                                    if ($_GET['del'] !== null) {
                                        if (mysqli_query($con, "DELETE FROM " . $prefix . "_sales where id=" . $_GET['del'] . "")) {
                                            echo '<div style="text-align:center; background-color:#E18C80; border-radius:5px;">
...
#####################################################
                                        if($_POST['inv_tax'] == '1'){
                                            ...
                                            $upsql="UPDATE " . $prefix . "_sales_inv SET paid='" . $_POST['paid'] . "',supplier=" . $_POST['SupplierID'] . ",edit_user=" . $user_id . ",shipping=" . $_POST['shipping'] . ",notes='" . $_POST['notes'] . "',tax=" . $taxValue . ",date='" . $date_edit . "',doc='" . $_POST['doc']. "',branch_id='" . $_POST['branch_id']. "',safe_id='" . $_POST['safe_id']. "',store_id='" . $_POST['store_id'] . "',discount='" . $_POST['alldiscount'] . "',PaymentMethod='" . $_POST['pay'] . "',staff='" . $_POST['staff'] . "'  where inv_id='" . $inv_id . "' and type='1'";
                                        mysqli_query($con, $upsql);




                                           $upsql="SELECT * From " . $prefix . "_sales_inv   where discount = (total*-1) and order_suppy_num='" . $inv_id . "' and supplier = '$data[supplier]' and type='3'";
                                           mysqli_query($con, $upsql);
                                    if(mysqli_affected_rows($con) > 0 ){
                                        $upsql="UPDATE " . $prefix . "_sales_inv SET  Total=" . ($_POST['alldiscount']*-1) . ",discount=" . ($_POST['alldiscount']) ." where discount = (total*-1) and order_suppy_num='" . $inv_id . "' and supplier = '$data[supplier]' and type='3'";
                                        mysqli_query($con, $upsql);
                                    }
                                    else{
                                          $upsql="SELECT * From " . $prefix . "_sales_inv   where discount = (total*-1) and  discount=$data[discount]  and  supplier = '$data[supplier]' and type='3'";

                                        mysqli_query($con, $upsql);
                                        $count = mysqli_affected_rows($con);
                                        if(($count  ) == 1 ) {
                                            $upsql="UPDATE " . $prefix . "_sales_inv SET  Total=" . ($_POST['alldiscount']*-1) . ",discount=" . ($_POST['alldiscount']) ." where discount = (total*-1) and discount=$data[discount]  and supplier = '$data[supplier]' and type='3'";
                                            mysqli_query($con, $upsql);
                                        }elseif(($count  ) == 0 ){
                                            $sqlpayments = "INSERT INTO " . $prefix . "_sales_inv (branch_id,order_suppy_num ,supplier, Total, date, PaymentMethod,paid, DueDate, notes, type, discount, staff,safe_id,store_id, doc, tax, user_id)
VALUES ('" . $_POST['branch_id'] . "','" . $data['inv_id'] . "','" . $_POST['SupplierID'] . "','" . (($_POST['alldiscount']) * -1) . "','" . $DueDate . "','1','" . $_POST['paid'] . "','','" . $Cash_discount_lang . "','3','" . $_POST['alldiscount'] . "','" . $_POST['staff'] . "','" . $_POST['safe_id'] . "','" . $_POST['store_id'] . "','" . $_POST['doc'] . "','" . $taxValue . "','" . $user_id . "')";
                                    ...
                                    if ( ( $_POST['paid'] != $data['paid']) and   $_POST['paid']>0) {




                                         $upsqlTreasury = "UPDATE " . $prefix . "_treasury SET Amount=" . $_POST['paid'] . "  where type = 4 and  inv_id=" . $inv_id . " ";
                                        mysqli_query($con, $upsqlTreasury);
                                        if (mysqli_affected_rows($con) == 0) {
                                            if ( $_POST['paid'] > 0) {
                                                $sqlt = "INSERT INTO " . $prefix . "_treasury (safe_id,type, Amount, date, notes, inv_id, inv_type, client_supp_name)
VALUES ('" . $_POST['safe_id'] . "','4','" .  $_POST['paid'] . "','" . $DueDate . "','تحصيلات نقدية','" . $inv_id . "','1','" . $_POST['SupplierID'] . "')";
                                        ...
                                        $upsql="UPDATE " . $prefix . "_sales_inv SET  Total=" . ($_POST['paid']*-1) . ",paid=" . $_POST['paid'] ." where order_suppy_num='" . $inv_id . "' and type='3'";
                                        mysqli_query($con, $upsql);
...
VALUES ('" . $_POST['branch_id'] . "','" .  $inv_id . "','" . $_POST['SupplierID'] . "','" . ( $_POST['paid'] * -1) . "','" . $DueDate . "','" . $pay . "','','" . $_POST['notes']  . "','3','" . $_POST['alldiscount'] . "','" . $_POST['staff'] . "','" . $_POST['safe_id']. "','" . $_POST['store_id']. "','" . $_POST['doc'] . "','" . $taxValue . "','" . $user_id . "')";
                                    ...
                                    }elseif ( ( $_POST['paid'] != $data['paid']) and   $_POST['paid']==0){
                                        $upsqlTreasury = "Delete FRom " . $prefix . "_treasury   where type = 4 and  inv_id=" . $inv_id . " ";

                                        mysqli_query($con, $upsqlTreasury);
                                         $upsql="Delete from " . $prefix . "_sales_inv   where order_suppy_num='" . $inv_id . "' and type='3'";
                                        ...
                                        while ($row_up = mysqli_fetch_array($result_up)) {
                                            ...
                                            if ($quantity == 0 or $quantity == ""  or $quantity == null) {
                                                $error_reports = 1;
                                                 ...
                                                 $sql="UPDATE " . $prefix . "_sales SET  Quantity='" . $quantity . "',Price='" . $Price . "',Discount='" . $Discount . "',
Total='" . $Total . "',type='1',BuyPrice='" . $BuyPrice . "',sales_type='" . $sales_type . "',size='" . $size . "',color='" . $color . "',user_id='$user_id',staff='$staff'  where id='" . $row_up['id'] . "'";
                                        ...
                                        header("refresh:1;url=sales_edit.php?id=".$_GET['id']."");
                                    }
                                    ###########################################
                                    if ($error_reports == "1") {
                                        echo '<div style="text-align:center; background-color:#E18C80; border-radius:5px;">
							...
							'.$quantity_not_zero_lang.'
                                    ...
                                    if (isset($_GET['SupplierID']) and $_GET['SupplierID'] !== null) {
                                        ...
                                        if (mysqli_query($con, "UPDATE ".$prefix."_sales SET SupplierID=".$SupplierID." where inv_id='$inv_id'")) {
                                            header("refresh:1;url=sales_edit.php?id=".$_GET['id']."");
                                        } else {
...
#####################
                                    ...
                                    if ($_GET['q'] == "d") {
                                        if (mysqli_query($con, "DELETE FROM " . $prefix . "_sales where user_id='$user_id'")) {
                                               header("refresh:1;url=sales_edit.php?id=".$_GET['id']."");
                                        }
                                    } else {
                                        if ($_GET['q'] == null) {
							...
							'.$item_not_found_lang.'
                                    ...
                                    <form method="post">
                                        ...
                                        <?php
//                                        $result_SupplierID = mysqli_query($con, "SELECT SupplierID FROM " . $prefix . "_sales where user_id='$user_id' limit 0,1");
//                                        if (@mysqli_num_rows($result_SupplierID) > 0) {
//                                            while ($row_SupplierID = mysqli_fetch_array($result_SupplierID)) {

                                        if (!$_POST['supplier']){
                                            $client_id = $data['supplier'] ;
                                        }else{
                                            $client_id = $_POST['supplier'] ;
                                        }
                                                 $ssqll = "SELECT id,name FROM " . $prefix . "_clients WHERE id=" . $client_id . "";
                                                $result_search_client = mysqli_query($con, $ssqll);
                                                if (@mysqli_num_rows($result_search_client) > 0) {
                                                    ...
                                                    while ($row_search_client = mysqli_fetch_array($result_search_client)) {
                                                        ...
                                                        echo'<input type="hidden" id="client_Val" name="SupplierID" value="' . $row_search_client['id'] . '" />';
                                                    }
                                                }
//                                            }
//                                        }
...
//                                            }
//                                        }
//                                        echo'<input type="hidden" name="SupplierID" value="' . $data['supplier'] . '" />';
//
//                                        ?>
                                        ...
                                        <table  border="1" style="font-size:16px; width:100%;  direction:rtl; border:1px; border-collapse:collapse; margin-top:10px; text-align:center;">
                                           ...
                                           <?php if($use_sizes==1){echo'<th class="text-center">'.$the_Size_lang.'</th>';} ?>
                                            ...
                                            <tbody>
                                                 ...
                                                 */
                                                $query = "SELECT COUNT(*) as num  FROM  " . $prefix . "_sales where inv_id='$inv_id' order by id DESC";
                                                ...
                                                /* Get data. */
                                                $sql = "SELECT * FROM " . $prefix . "_sales where inv_id='$inv_id' order by id DESC";
...
###############################################################################
                ...
                while ($row_colors = mysqli_fetch_array($result_colors)) {
                    if ($row['color'] == $row_colors['id']) {
                        ...
                        echo '<option value="' . $row_colors['id'] . '">' . $row_colors['color'] . '</option >';
							...
							<td valign="middle"><a href="sales_edit.php?id=' . $_GET['id'] . '&del=' . $row['id'] . '"><img src="images/erase.png" style="border:0px;" /></a></td>
							</tr>';
                                                $i++;
                                            }
                                            ?>
    ...
    echo"<tr>";
    if($db_tax>0 and ($_POST['inv_tax'] == '1' or $data['tax'] > 0)){
    ...
    echo'<td  colspan="5">'.$tax_lang.'</td>
     ...
     echo'<tr>';
                            ...
                            echo '</tr>';
                              #############################################
    if($_POST['inv_tax'] == '1'){
        $taxValue=(($db_tax/(100+$db_tax))*(($sumTotal - $alldiscount)));
    }else{
        $taxValue = 0 ;
    }
     mysqli_query($con, "UPDATE " . $prefix . "_sales_inv SET Total='" . $sumTotal  . "' ,tax = ".$taxValue."  where inv_id='" . $inv_id . "' and type='1'");
    ...
    ?>
                                ...
                                <table width="100%" dir="rtl">
                                    <tr>
                                        <td hidden style="font-size:16px;"><select name="pay"   style="text-align:center;background-color:#CCC; float:right;">
                                                <option value="1" <?php
                                        ...
                                        <td ><?php echo"$amount_paid_lang"; ?></td>
                                        <td  style="font-size:16px;"><input type="text" name="paid" value="<?php  echo $data[paid]; ?>"  style="text-align:center; background-color:#CCC; width:50px; height:20px;"/></td>
                                        <td><?php echo"التاريخ"; ?></td>
                                        ...
                                        <td hidden style="font-size:16px;" class="text-center">م.النقل</td>
                                        <td hidden style="font-size:16px;"><input   style="text-align:center; background-color:#CCC; width:80px; height:20px;" type="text" name="shipping" value="<?php echo"" . $data[shipping] . ""; ?>" /></td>
                                    ...
                                    <tr>
                                        ...
                                        <td hidden style="font-size:16px;"  class="text-left"><?php echo"الضرائب"; ?></td>

<!--                                        --><?php
//
//                                       if($data['tax'] > 0 or $_POST['inv_tax'] == '1'){
//                                           $tax = $data['tax'];
//                                           ?>
<!--                                           <td><input checked="checked" type="checkbox" name="inv_tax" id="inv_tax" value="1" --><?php //if($_POST['inv_tax']==null or $_POST['inv_tax']=="0"){}else{echo' checked="checked"';} ?><!-- /></td>-->
<!---->
<!--                                           --><?php
//
//                                       }
...
//                                           $tax=$_POST['inv_tax'];
//                                           ?>
<!--                                           <td><input type="checkbox" name="inv_tax" id="inv_tax" value="1" --><?php //if($_POST['inv_tax']==null or $_POST['inv_tax']=="0"){}else{echo' checked="checked"';} ?><!-- /></td>-->
<!---->
<!--                                           --><?php
//
//                                       }
//                                       ?>
                                        ...
                                        <td hidden style="font-size:16px;"><?php echo"$the_Employee"; ?></td>
                                        <td hidden><select name="staff"  style="text-align:center;background-color:#CCC; float:right;">
<option value="" <?php if($data['staff']=="" or $data['staff']==null){echo "selected='selected'";}else{} ?>></option>
<?php
...
while($row_staff = mysqli_fetch_array($result_staff))
  {
    if($row_staff['id']==$data['staff']){$select="selected='selected'";}else{$select=null;}
...
?>
                                        ...
                                        <td hidden style="font-size:16px;"><?php echo"DOC#"; ?></td>
                                        <td hidden><input type="text" name="doc" value="<?php echo"" . $data['doc'] . ""; ?>"  style="text-align:center; background-color:#CCC; width:50px; height:20px;"/></td>
                                    </tr>
                                    <tr>
