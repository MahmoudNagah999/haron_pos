	...
	<table border="1"
		...
		<thead>
			<td colspan="7" style="background-color:#09F;"><strong style="color:#FFF; font-size:22px;">
					<?php echo "رصيد الخزينة"; ?></strong></td>
		</thead>
		...
		*/
		$query = "SELECT COUNT(*) as num  FROM  " . $prefix . "_treasury where (Amount + 0) > 0 and left(date, 10) BETWEEN '" . $from . "' AND '" . $to . "'";
		...
		$sql = "SELECT * FROM " . $prefix . "_treasury where (Amount + 0) > 0 and left(date,10) BETWEEN '" . $from . "' AND '" . $to . "' order by $orderby $type LIMIT $start, $limit";
		...
		*/
		...
		<thead style="background-color:#CCC;">
			...
			$result_get = mysqli_query($con, "SELECT Amount FROM " . $prefix . "_treasury where (Amount + 0) > 0 and left(date,10) BETWEEN '" . $from . "' AND '" . $to . "'");
