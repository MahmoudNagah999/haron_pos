...
<thead>
<td colspan="7" style="background-color:#09F;"><strong style="color:#FFF; font-size:22px;">  <?php echo"$Return_items_lang"; ?> / <?php echo"".$_GET['item'].""; ?> <?php echo"$from_lang"; ?> <?php echo"".$_GET['from'].""; ?> <?php echo"$to_lang"; ?> <?php echo"".$_GET['to'].""; ?></strong></td>
</thead>
  ...
  <th class="text-center"><a href="#" class="a_remove_underlines"> <?php echo"$Code_lang"; ?> </a></th>
    <th class="text-center"><a href="#" class="a_remove_underlines"> <?php echo"$the_items_lang"; ?> </a></th>
      <th class="text-center"><a href="#" class="a_remove_underlines"> <?php echo"$the_Quantity_lang"; ?> </a></th>
        <th class="text-center"><a href="#" class="a_remove_underlines"><?php echo"$Selling_price_lang"; ?></a></th>
...
$to=stripslashes(date('Y-m-d',strtotime($to)));
	$tbl_name="".$prefix."_sales";		//your table name
	...
	*/
	$query = "SELECT COUNT(*) as num  FROM  ".$prefix."_sales where (Total + 0) > 0 and item='".$_GET['q']."' and type='2' and  left(date, 10) BETWEEN '$from' AND '$to'";
	$total_pages = @mysqli_fetch_array(mysqli_query($con,$query));
	$total_pages = $total_pages['num'];
		
	/* Setup vars for query. */
	$targetpage = "?q=".$_GET['q']."&from=".$_GET['from']."&to=".$_GET['to']."&limit=".$_GET['limit']."&orderby=".$_GET['orderby']."&type=".$_GET['type']."&reports=items_returns"; 	//your file name  (the name of this file)
...
$sql = "SELECT * FROM ".$prefix."_sales where (Total + 0) > 0 and item='".$_GET['q']."' and type='2' and left(date,10) BETWEEN '$from' AND '$to' order by date DESC LIMIT $start, $limit";	
...
$result_get = mysqli_query($con,"SELECT Total FROM ".$prefix."_sales where (Total + 0) > 0 and item='".$_GET['q']."' and type='2' and left(date,10) BETWEEN '".$from."' AND '".$to."'");
