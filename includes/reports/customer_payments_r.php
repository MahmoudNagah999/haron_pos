                                                <?php
			if($user_CustomerReports!=="1" and $user_IsAdmin!=1){
				
				echo '<div class="alert alert-warning text-right" style="margin-top:150px;">                   '.$not_have_permission_lang.'                             </div>';
				}else{if($_GET['clientID']!==""){ ?>
<table border="1" style="font-size:16px; width:100%; direction:rtl; border:1px; border-collapse:collapse;  text-align:center;">
<thead>
<td colspan="7" style="background-color:#09F;"><strong style="color:#FFF; font-size:22px;"> <?php echo"$Customer_payments_lang"; ?>/ <?php echo"".$_GET['client'].""; ?> - <?php echo"$from_lang"; ?> <?php echo"".$_GET['from'].""; ?> <?php echo"$to_lang"; ?> <?php echo"".$_GET['to'].""; ?></strong><a href="#" onclick="javascript:void window.open('statement_of_account_client.php?from=<?php echo"".$_GET['from'].""; ?>&to=<?php echo"".$_GET['to'].""; ?>&id=<?php echo"".$_GET['clientID'].""; ?>','1390937502641','width=1024,height=600,toolbar=0,menubar=0,location=0,status=0,scrollbars=1,resizable=0,left=0,top=0,menubar=0');return false;" style="float:left;"><img src="images/arrears_list.gif" style="border:0px;" title="كشف حساب العميل" /></a></td>
</thead>
  <thead style="background-color:#CCC;">

  <th  class="text-center <?php if($_GET['type']=="ASC" and $_GET['orderby']=="id"){echo"sort_t";}else if($_GET['type']=="DESC" and $_GET['orderby']=="id"){echo"sort_d";}else{echo"sort0";}?>"><a href="?SupplierID=<?php echo"".$_GET['SupplierID'].""; ?>&reports=<?php echo"".$_GET['reports'].""; ?>&from=<?php echo"".$_GET['from'].""; ?>&to=<?php echo"".$_GET['to'].""; ?>&orderby=id&type=<?php if($_GET['type']=="ASC"){echo"DESC";}else if($_GET['type']=="DESC"){echo"ASC";}else{echo"DESC";} ?>&page=<?php echo"".$_GET['page'].""; ?>" class="a_remove_underlines"> <?php echo"$Code_lang"; ?> </a></th>
...
$to=stripslashes(date('Y-m-d',strtotime($to)));
	...
	*/
	$query = "SELECT COUNT(*) as num  FROM  ".$prefix."_sales_inv  where (Total + 0) > 0 and supplier='".$_GET['clientID']."' and left(date, 10) BETWEEN '$from' AND '$to'";
	$total_pages = @mysqli_fetch_array(mysqli_query($con,$query));
	$total_pages = $total_pages['num'];
		
	/* Setup vars for query. */
	$targetpage = "?from=".$_GET['from']."&to=".$_GET['to']."&limit=".$_GET['limit']."&orderby=".$_GET['orderby']."&type=".$_GET['type']."&reports=customer_payments"; 	//your file name  (the name of this file)
...
$sql = "SELECT * FROM ".$prefix."_sales_inv where (Total + 0) > 0 and supplier='".$_GET['clientID']."' and type='3' and left(date,10) BETWEEN '$from' AND '$to' order by $orderby $type LIMIT $start, $limit";	
...
$result_suppliers = mysqli_query($con,"SELECT id,name FROM ".$prefix."_clients WHERE id=".$_GET['clientID']."");
...
if($row['PaymentMethod']==2){$PaymentMethod="$On_credit_lang";}
...
$result_get = mysqli_query($con,"SELECT Total FROM ".$prefix."_sales_inv where (Total + 0) > 0 and supplier='".$_GET['clientID']."' and type='3' and left(date,10) BETWEEN '".$from."' AND '".$to."'");
