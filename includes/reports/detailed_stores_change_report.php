<?php
if ($user_stores_changeReports !== "1" and $user_IsAdmin != 1) {
    ...
    <table border="1"
        ...
        <thead>
            <td colspan="6" style="background-color:#09F;"><strong style="color:#FFF; font-size:22px;">
                    <?php echo "تقرير تحويل المخازن التفصيلى"; ?></strong></td>
        </thead>
        ...
        $tbl_name = "stores_change";		//your table name
        // How many adjacent pages should be shown on each side?
        $adjacents = 3;
        $query = "SELECT COUNT(*) as num  FROM  " . $prefix . "_stores_change where $quy left(date, 10) BETWEEN '$from' AND '$to'";
        $total_pages = @mysqli_fetch_array(mysqli_query($con, $query));
        $total_pages = $total_pages['num'];

        /* Setup vars for query. */
        $targetpage = "?q=" . $_GET['q'] . "&groupid=" . $_GET['groupid'] . "&from=" . $_GET['from'] . "&to=" . $_GET['to'] . "&limit=" . $_GET['limit'] . "&orderby=" . $_GET['orderby'] . "&type=" . $_GET['type'] . "&reports=detailed_stores_change"; 	//your file name  (the name of this file)
        ...
        $sql = "SELECT *,left(date,10) as date FROM " . $prefix . "_stores_change where $quy left(date,10) BETWEEN '$from' AND '$to' order by date DESC LIMIT $start, $limit";
...
-->
        ...
        <thead style="background-color:#CCC;">
            ...
            $result_get = mysqli_query($con, "SELECT Total FROM " . $prefix . "_stores_change where $quy left(date,10) BETWEEN '" . $from . "' AND '" . $to . "'");
        ...
        <tr>
            <td class="text-center" colspan="5"><?php echo "$pagination"; ?></td>
        </tr>
    </table>
<?php } ?>