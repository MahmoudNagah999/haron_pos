...
<thead>
<td colspan="7" style="background-color:#09F;"><strong style="color:#FFF; font-size:22px;"> <?php echo"$Purchases_lang"; ?></strong></td>
</thead>
...
$to=stripslashes(date('Y-m-d',strtotime($to)));
	...
	*/
	$query = "SELECT COUNT(*) as num  FROM  ".$prefix."_receivings_inv  where Total > 0 and supplier='".$_GET['SupplierID']."' and type='1' and  left(date, 10) BETWEEN '$from' AND '$to'";
	$total_pages = @mysqli_fetch_array(mysqli_query($con,$query));
	$total_pages = $total_pages['num'];
		
	/* Setup vars for query. */
	$targetpage = "?SupplierID=".$_GET['SupplierID']."&from=".$_GET['from']."&to=".$_GET['to']."&limit=".$_GET['limit']."&orderby=".$_GET['orderby']."&type=".$_GET['type']."&reports=suppliers_receivings"; 	//your file name  (the name of this file)
 ...
 $sql = "SELECT * FROM ".$prefix."_receivings_inv where Total > 0 and supplier='".$_GET['SupplierID']."' and type='1' and left(date,10) BETWEEN '$from' AND '$to' order by $orderby $type LIMIT $start, $limit";
...
$result_get = mysqli_query($con,"SELECT Total,discount FROM ".$prefix."_receivings_inv where Total > 0 and supplier='".$_GET['SupplierID']."' and type='1' and left(date,10) BETWEEN '".$from."' AND '".$to."'");
