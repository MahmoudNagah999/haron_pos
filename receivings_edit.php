...
$inv_id=$_GET['id'];
$data=get_receivings_inv_data($inv_id,'1');
if($data[id]==null){
...
if (isset($_GET['cat_show'])) {
    ...
    header("refresh:0;url=receivings_edit.php?id=".$_GET['id']."");
}
?>
<!DOCTYPE html>
<html lang="en">
									...
									//	echo ($inv_Totalt);
                                        if($db_tax>0){
                                         $total_inv_after_alldiscount_and_tax=($inv_Totalt - $alldiscount)+(($inv_Totalt - $alldiscount)*$db_tax/100);
                                          ...
                                          $upsql="SELECT * From " . $prefix . "_receivings_inv   where discount = (total*-1) and inv_id='" . $inv_id . "' and supplier = '$data[supplier]' and type='3'";
                                        mysqli_query($con, $upsql);
                                        if(mysqli_affected_rows($con) > 0 ){
                                            $upsql="UPDATE " . $prefix . "_receivings_inv SET  Total=" . ($_POST['alldiscount']*-1) . ",discount=" . ($_POST['alldiscount']) ." where discount = (total*-1) and inv_id='" . $inv_id . "' and supplier = '$data[supplier]' and type='3'";
                                            mysqli_query($con, $upsql);
                                        }
                                        else{
                                             $upsql="SELECT * From " . $prefix . "_receivings_inv   where discount = (total*-1) and  discount=$data[discount]  and  supplier = '$data[supplier]' and type='3'";

                                            mysqli_query($con, $upsql);
                                            $count = mysqli_affected_rows($con);
                                            if(($count  ) == 1 ) {
                                                $upsql="UPDATE " . $prefix . "_receivings_inv SET  Total=" . ($_POST['alldiscount']*-1) . ",discount=" . ($_POST['alldiscount']) ." where discount = (total*-1) and discount=$data[discount]  and supplier = '$data[supplier]' and type='3'";
                                                mysqli_query($con, $upsql);
                                            }elseif(($count  ) == 0 ){
...
VALUES ('" . $_POST['branch_id'] . "','" . $data['inv_id'] . "','" . $_POST['SupplierID'] . "','" . (($_POST['alldiscount']) * -1) . "','" . $DueDate . "','1','" . $_POST['paid'] . "','','" . $Cash_discount_lang . "','3','" . $_POST['alldiscount'] . "','"  . $_POST['safe_id'] . "','" . $_POST['store_id']   . "')";
...
//                                                echo '<br/>';
                                                ...
                                                if ($quantity == 0 or $quantity == "" or $quantity == null) {
                                                    $error_reports = 1;
                                                     ...
                                                     $sql = "UPDATE " . $prefix . "_receivings SET  Quantity='" . $quantity . "',Price='" . $Price . "',Discount='" . $Discount . "',
...
#####################################################
mysqli_query($con, "UPDATE " . $prefix . "_receivings_inv SET date='" . $date_edit . "',safe_id='" . $_POST['safe_id'] . "',store_id='" . $_POST['store_id']. "',doc='" . $_POST['doc'] . "',discount='" . $_POST['alldiscount']  . "',paid='" . $_POST['paid'] . "',Total='". $total_inv ."'  where inv_id='" . $inv_id . "' and type='1'");
//mysqli_errno($con);
#####################################################
                                        header("refresh:1;url=receivings_edit.php?id=".$_GET['id']."");
                                    }
                                    ###########################################
                                    if ($error_reports == "1") {
                                        echo '<div style="text-align:center; background-color:#E18C80; border-radius:5px;">
...
//                                            }else{
							...
							VALUES ('" . $item_id_new . "','" . $item_Retail_price_new . "','1','" . $row_new['Discount'] . "','" . $item_total_new . "','1','" . $DueDate . "','" . $item_price_new . "','" . $item_subqty . "','" . $size . "','" . $color . "','".$user_id."','".$inv_id."')";
                                                    }
                                                    if (!mysqli_query($con, $sql)) {
                                                        echo '<div style="text-align:center; background-color:#E18C80; border-radius:5px;">
							<span style="float:left; padding-left:20px;"><img src="images/chmarkicon.png" style="border:0px;" /></span>
...
//                                        }
                                    ...
                                    <form method="post">
                                        <?php
                                                        echo" <span dir=\"rtl\"  style=\"color:#7D060F;\" dir=\"rtl\">$The_Supplier_lang :".get_supplier_data($data[supplier])[name]."</span>";
                ...
                while ($row_colors = mysqli_fetch_array($result_colors)) {
                    if ($row['color'] == $row_colors['id']) {
                        ...
                        echo '<option value="' . $row_colors['id'] . '">' . $row_colors['color'] . '</option >';
							...
							<td valign="middle"><a href="receivings_edit.php?id=' . $_GET['id'] . '&del=' . $row['id'] . '"><img src="images/erase.png" style="border:0px;" /></a></td>
							</tr>';
                                                $i++;
                                            }
                                            ?>
    ...
    echo'<td  colspan="5">'.$tax_lang.'</td>
     ...
     echo'<tr>';
                            ...
                            <td><font style="color:#060; font-weight:bold;">'.((($sumTotal-$print_total_disc)*$db_tax/100)+($sumTotal-$print_total_disc)+$data[shipping]).'</font></td><td></td>
                              ...
                              #############################################
if(isset($_GET['q'])){
     mysqli_query($con, "UPDATE " . $prefix . "_receivings_inv SET Total='" . $sumTotal . "'  where inv_id='" . $inv_id . "' and type='1'");
}
