...
while($row_search_clients = mysqli_fetch_array($result_search_clients))
  ...
  echo'<li class="'.$class.'"><a class="addProd" par="q" attr="'.$row_search_clients['id'].'" >'.$row_search_clients['item'].' ('.$whole.','.round($all_qty).')</a></li>';
  $i++;
   }
}else{
    $no1 = 1;
...
//	$i=1;
        while ($row_search_clients = mysqli_fetch_array($result_search_clients)) {
            ...
            echo '<li class="' . $class . '"><a  class="addProd" par="q" attr="'.$row_search_clients['id'].'">' . $row_search_clients['name'] . '(Offer)' . ')</a></li>';
            $i++;
        }
    } else {
