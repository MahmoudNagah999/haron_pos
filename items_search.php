...
while($row_search_clients = mysqli_fetch_array($result_search_clients))
  ...
  echo'<li class="'.$class.'"><a href="?id='.$id.'&q='.$row_search_clients['id'].'&item_type=items&order_id='.$_GET['order_id'].'&order_user_id='.$_GET['order_user_id'].'">'.$row_search_clients['item'].' ('.$whole.','.round($all_qty).')</a></li>';
  $i++;
   }
}else{
    $no1 = 1;
...
//	$i=1;
        while ($row_search_clients = mysqli_fetch_array($result_search_clients)) {
            ...
            echo '<li class="' . $class . '"><a href="?id=' . $id . '&q=' . $row_search_clients['id'] . '&item_type=' . $prefix . '_offers_inv&order_id=' . $_GET['order_id'] . '&order_user_id=' . $_GET['order_user_id'] . '">' . $row_search_clients['name'] . '(Offer)' . ')</a></li>';
            $i++;
        }
    } else {
