...
//    }
    ...
    $result_order_supply_status = @mysqli_query($con, "SELECT user_id,status,COUNT(*) as counts FROM " . $prefix . "_order_supply_inv where id !='0' $q_status $q_region $q_branch $sql_date GROUP BY user_id,status");
...
$schema_insert .= '<td><u>الموظف</u></td>';
...
if (empty($users)) {
    ...
    if ($users == '') {
        ...
        $q_users = "where id in ($users)";
    }
}
$result = @mysqli_query($con, "SELECT * FROM cairo_users $q_users");
