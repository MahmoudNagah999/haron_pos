...
if (isset($_GET['cat_show'])) {
    ...
    header("refresh:0;url=sales_returns_edit.php?id=".$_GET['id']."");
}
########################
if (isset($_GET['SupplierID'])) {
    mysqli_query($con, "UPDATE " . $prefix . "_sales_inv SET supplier=" . $_GET['SupplierID'] . " where inv_id=" . $inv_id . " and type='2'");
    header("refresh:0;url=sales_returns_edit.php?id=".$_GET['id']."");
}
########################
if (isset($_GET['SupplierID'])) {
    mysqli_query($con, "UPDATE " . $prefix . "_sales SET SupplierID=" . $_GET['SupplierID'] . " where inv_id=" . $inv_id . " and type='2'");
    header("refresh:0;url=sales_returns_edit.php?id=".$_GET['id']."");
}
?>
<!DOCTYPE html>
    ...
    <head>
        ...
        <script>
            function showResults(str) {
                ...
                xmlhttp.open("GET", "clients_search.php?id=<?php echo $_GET['id']; ?>&q=" + str, true);
                xmlhttp.send();
            }
        </script>
        <script>
                    ...
                    <div class="noprint">
                        <div style="width:60%;border:1px solid #CCC; border-radius:5px; float:right; ">
                            ...
                            <div>
                                ...
                                <table class="text-right" style="width: 100%; text-align: right; border: 2px;">
                                    <tr>
                                        <td dir="rtl" class="text-right"><b> <?php echo get_client_data($data['supplier'])[name]; ?> </b></td>
                                        <td dir="rtl" class="text-right" style="width:25px;"><b> الاسم/ </b></td>
                                    </tr>
                                </table>
...
#####################################################
//                                        var_dump($total_inv_after_alldiscount_and_tax);
//                                        die('------------');

//mysqli_errno($con);


                                        $upsql="SELECT * From " . $prefix . "_sales_inv   where discount = (total) and order_suppy_num='" . $inv_id . "' and supplier = '$data[supplier]' and type='3'";
                                        mysqli_query($con, $upsql);
                                        if(mysqli_affected_rows($con) > 0 ){
                                            $upsql="UPDATE " . $prefix . "_sales_inv SET  Total=" . ($_POST['alldiscount']) . ",discount=" . ($_POST['alldiscount']) ." where discount = (total) and order_suppy_num='" . $inv_id . "' and supplier = '$data[supplier]' and type='3'";
                                            mysqli_query($con, $upsql);
                                        }
                                        else{
                                            $upsql="SELECT * From " . $prefix . "_sales_inv   where discount = (total) and  discount=$data[discount]  and  supplier = '$data[supplier]' and type='3'";

                                            mysqli_query($con, $upsql);
                                            $count = mysqli_affected_rows($con);
                                            if(($count  ) == 1 ) {
                                                $upsql="UPDATE " . $prefix . "_sales_inv SET  Total=" . ($_POST['alldiscount']) . ",discount=" . ($_POST['alldiscount']) ." where discount = (total) and discount=$data[discount]  and supplier = '$data[supplier]' and type='3'";
                                                mysqli_query($con, $upsql);
                                            }elseif(($count  ) == 0 ){
...
VALUES ('" . $_POST['branch_id'] . "','" . $data['inv_id'] . "','" . $_POST['SupplierID'] . "','" . (($_POST['alldiscount']) ) . "','" . $DueDate . "','1','" . $_POST['paid'] . "','','" . $Cash_discount_lang . "','3','" . $_POST['alldiscount'] . "','" . $_POST['staff'] . "','" . $_POST['safe_id'] . "','" . $_POST['store_id'] . "','" . $_POST['doc'] . "','" . $taxValue . "','" . $user_id . "')";
                                        ...
                                        while ($row_up = mysqli_fetch_array($result_up)) {
                                         ...
                                         //   echo"<br />";
                                            if ($quantity == 0 or $quantity == ""  or $quantity == null) {
                                                ...
                                                mysqli_query($con, "UPDATE " . $prefix . "_sales SET  Quantity='" . ($quantity) . "',Price='" . $Price . "',Discount='" . $Discount . "',
Total='" . $Total . "',type='1',BuyPrice='" . $BuyPrice . "',sales_type='" . $sales_type . "',size='" . $size . "',color='" . $color . "',user_id='$user_id',staff='$staff'  where id='" . $row_up['id'] . "'");
                                        ...
                                        $sql ="UPDATE " . $prefix . "_sales_inv SET Total='" . $Total . "', shipping=" . $_POST['shipping'] . ",date='" . $date_edit . "',doc='" . $_POST['doc'] . "',discount='" . $_POST['alldiscount'] . "',PaymentMethod='" . $_POST['pay'] . "',staff='" . $_POST['staff'] . "',paid='" . $_POST['paid'] . "'  where inv_id='" . $inv_id . "' and type='2'";
                                        mysqli_query($con , $sql);   header("refresh:1;url=sales_returns_edit.php?id=".$_GET['id']."");
                                    }
                                    ###########################################
                                    if ($error_reports == "1") {
                                        echo '<div style="text-align:center; background-color:#E18C80; border-radius:5px;">
							...
							'.$quantity_not_zero_lang.'
                                    ...
                                    if (isset($_GET['SupplierID']) and $_GET['SupplierID'] !== null) {
                                        ...
                                        if (mysqli_query($con, "UPDATE ".$prefix."_sales SET SupplierID=".$SupplierID." where inv_id='$inv_id'")) {
                                            header("refresh:1;url=sales_returns_edit.php?id=".$_GET['id']."");
                                        } else {
                ...
                while ($row_colors = mysqli_fetch_array($result_colors)) {
                    if ($row['color'] == $row_colors['id']) {
                        ...
                        echo '<option value="' . $row_colors['id'] . '">' . $row_colors['color'] . '</option >';
							...
							<td valign="middle"><a href="sales_returns_edit.php?id=' . $_GET['id'] . '&del=' . $row['id'] . '"><img src="images/erase.png" style="border:0px;" /></a></td>
							</tr>';
                                                $i++;
                                            }
                                            ?>
    ...
    echo'<td  colspan="5">'.$tax_lang.'</td>
     ...
     echo'<tr>';
                            ...
                            <td><font style="color:#060; font-weight:bold;">'.((($sumTotal-$print_total_disc)*$db_tax/100)+($sumTotal-$print_total_disc)+$data[shipping]).'</font></td><td></td>
                              ...
                              #############################################
if(isset($_GET['q'])){
     mysqli_query($con, "UPDATE " . $prefix . "_sales_inv SET Total='" . $sumTotal . "',tax='" . (($sumTotal-$print_total_disc)*$db_tax/100) . "'  where inv_id='" . $inv_id . "' and type='2'");
}
    ...
    ?>
                                ...
                                <table width="100%" dir="rtl">
                                    <tr>
                                        <td style="font-size:16px;"><select name="pay"   style="text-align:center;background-color:#CCC; float:right;">
                                                <option value="1" <?php
                                        ...
                                        <td style="font-size:16px;" class="text-center">م.النقل</td>
                                    ...
                                    <tr>
                                        ...
                                        <td style="font-size:16px;"><?php echo"$the_Employee"; ?></td>
                                        <td><select name="staff"  style="text-align:center;background-color:#CCC; float:right;">
...
?>
                                                                  ...
                                                                  <td style="font-size:16px;"><?php echo"DOC#"; ?></td>
                                        <td><input type="text" name="doc" value="<?php echo"" . $data['doc'] . ""; ?>"  style="text-align:center; background-color:#CCC; width:50px; height:20px;"/></td>
                                    </tr>
                                    <tr>
