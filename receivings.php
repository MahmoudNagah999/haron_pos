...
<head>
    ...
    <script>
        function showResults(str) {
            ...
            xmlhttp.open("GET", "ajax/suppliers_search_ajax.php?q=" + str, true);
            xmlhttp.send();
        }
    </script>
    <script>
...
//                        if($_POST['SupplierID']=="" and $paid!=($inv_Totalt-$alldiscount))	{
                            ...
                            if($paid<0 or $paid==null or $paid==""){}else{
                                $sqlpayments_suppliers="INSERT INTO ".$prefix."_receivings_inv (supplier, Total, date, PaymentMethod, DueDate, notes, type,safe_id,store_id, discount)
VALUES ('".$_POST['SupplierID']."','".($paid*-1)."','".$DueDate."','".$pay."','','".$notes."','3','" . $_POST['safe_id'] . "','". $_POST['store_id'] . "','".$_POST['alldiscount']."')";
                            ...
                            if($alldiscount<=0){}else{
                                //	echo"0000000000000";
                                $sqlpayments="INSERT INTO ".$prefix."_receivings_inv (supplier, Total, date, PaymentMethod, DueDate, notes, type,safe_id,store_id, discount)
VALUES ('".$_POST['SupplierID']."','".(($alldiscount)*-1)."','".$DueDate."','".$pay."','','".$Cash_discount_lang."','3','" . $_POST['safe_id'] . "','". $_POST['store_id'] . "','".$_POST['alldiscount'] . "')";
...
VALUES ('".$user_id."','".$_POST['safe_id']."','3','" . ($paid*-1) . "','" . $DueDate . "','مدفوعات موردين','" . $get_LastInvoiceNo . "','2','" . $_POST['SupplierID'] . "')";
...
###########################################
                            ...
                            $sqlIN="INSERT INTO ".$prefix."_receivings_inv(inv_id, date, Total, supplier, PaymentMethod, DueDate, CheckNumber, notes, type, paid,safe_id,store_id, discount)
VALUES ('".$get_LastInvoiceNo."','".$DueDate."','".$inv_Total."','".$_POST['SupplierID']."','".$pay."','','".$CheckNumber."','".$notes."','1','".$paid."','" . $_POST['safe_id']."','" . $_POST['store_id'] . "','".$_POST['alldiscount'] . "')";
...
#####################
                    ...
                    <form method="post" id="inv_form">
                        ...
                        <div style="font-size:16px;"><span dir="rtl"><?php echo $The_Supplier_lang ; ?>: <br /></span>
                            <span style="color:#7D060F;" dir="rtl"><div id="ajaxResSupp">
                                            <?php echo get_supplier_data( $_POST['SupplierID'] )[name]; ?>
                                </div></span></div>
...
###############################################################################
							...
							<td><input  class="'.$gr.'" type="number" name="quantity'.$row['id'].'"  value="'.$row['Quantity'].'" style="width:10%; height:20px; text-align:center;border:0px;"/></td>';
                                ...
                                echo'<td><input tabindex = "'.$i.'"  class="'.$gr.'" type="text"  name="price'.$row['id'].'"  value="'.$row['BuyPrice'].'"  style="width:10%; height:20px;text-align:center;border:0px;" /></td>
					...
					';
                            ...
                            <td colspan="5">الخصم</td>';
                                ...
                                echo'<td><font style="color:#060; font-weight:bold;">'.($sumTotal*($_POST['alldiscount']/100)).'</font></td>';
                                echo'<td></td>';
                                echo' </tr>';
...
<a class="addProd" par="cat_show" attr="0" ><i class="fa fa-list" title="'.$All_groups_lang.'"></i>
</a></span>';
                        }
                        ?>
                </form></div>
                ...
                while ($row = @mysqli_fetch_array($result)) {
                    ...
                    if ($row['image']== null or $row['useimage'] == 0) {
                        ...
                        echo "<div class=\"draggable-demo-product jqx-rc-all\">
							...
							<a class='addProd' par='q' attr='".$row['id']."'><img src=\"uploads/".$row['image']."\" class=\"img-responsive\" width=\"115\" height=\"115\" /></a>
							</div>";
                    }
...
<script>
    ...
    $(document).on("change",'.gr,.gr2',function() {
        ...
        $.ajax({
            url: 'ajax/receivings_add_product_temp.php',
            dataType: 'text',
            type: 'POST',
    ...
    $(document).on("click",'.addSupplier', function() {
        ...
        $.ajax({
            url: 'ajax/ajax_update_supp_recivies.php',
            dataType: 'text',
            type: 'GET',
            contentType: 'application/x-www-form-urlencoded',
            data: {'SupplierID':attr_id},
            success: function (data, textStatus, jQxhr) {
                data = JSON.parse(data);
            },
            error: function (jqXhr, textStatus, errorThrown) {
